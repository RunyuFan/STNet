...
def main(args):
    ...
    # cost2_2 = CB_loss(0.9999, 2.0).to(device)
    cost2 = nn.CrossEntropyLoss().to(device)  # CB_loss(beta, gamma)
    ...
    # scheduler2=lr_scheduler.ReduceLROnPlateau(optimizer2, mode='min', factor=0.9, patience=4, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0.000001, eps=1e-08)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, msi, checkin, labels in train_loader:
            ...
            _, outputs2 = model2(images, msi, checkin)
            # outputs3 = model3(images)
            # loss1 = cost1(outputs1, labels)
            # print(outputs2.shape)
            loss2 = cost2(outputs2, labels)
            # loss2 = CB_loss(outputs2, labels, samples_per_cls, no_of_classes,loss_type, beta, gamma)
        ...
        if epoch % 1 == 0:
            ...
            print("Total Acc Model CheckinNet: %.4f" % (correct_prediction_2 / total_2))
            print('----------------------------------------------------')

        if acc_2 > best_acc_2:
            print('save new best acc_2', acc_2)
            torch.save(model2, os.path.join(args.model_path, 'STNet-6-2-2.pth'))
            best_acc_2 = acc_2
            best_epoch = epoch
        # if acc_3 > best_acc_3:
