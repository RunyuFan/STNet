...
class ResNeXt(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x_layer4.shape)

        x = self.avgpool(x_layer4)
        x = x.view(x.size(0), -1)
        x = self.fc(x)

        return x_layer1, x_layer2, x_layer3, x_layer4, x
...
if __name__ == '__main__':
    ...
    model = resnext50()
    x_layer1, x_layer2, x_layer3, x_layer4, x = model(x)
    print(x_layer1.shape, x_layer2.shape, x_layer3.shape, x_layer4.shape, x.shape)
